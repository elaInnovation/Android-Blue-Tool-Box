...
public class TemperatureFragment extends Fragment
{
    ...
    private TextView tagName, tagValueTemp, tagConnectName, tagStateConnection, tagBatValue;
    private GraphView graphView;
    private LinearLayout layoutConnection;
    private View viewLimit, viewEnd;
    private LineGraphSeries dataSerie = new LineGraphSeries<DataPoint>();
    private PointsGraphSeries<DataPoint> pointSerie = new PointsGraphSeries<>();
    ...
    private void mapUi(View v)
    {
        ...
        this.dataSerie.setDrawBackground(true);
        this.dataSerie.setBackgroundColor(Color.argb(40, 51, 102, 153));

        this.graphView.addSeries(dataSerie);
        this.graphView.addSeries(pointSerie);
    ...
    private void updateUi(LinkedHashMap<String, Tag> tagList, LinkedHashMap<String, String> macList)
    {
        ...
        {
            if(entry.getValue() instanceof TagTemperature) {
    ...
    private void updateTableRow(String name, Tag tag)
    {
        ...
        if(tag.getClass().equals(TagTemperature.class) &&
                this.tagSelectedName.equals(tag.getName())) {
            this.dataSerie.appendData(new DataPoint(this.indexEmission, ((TagTemperature)tag).getTemperature()), true, 40);
            this.pointSerie.appendData(new DataPoint(this.indexEmission, ((TagTemperature)tag).getTemperature()), true, 40);
            this.indexEmission++;
        }
    }
    ...
    private void newTableRow(final String name, final String mac, int rssi)
    {
        ...
        tableRow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                dataSerie.resetData(new DataPoint[] {});
                pointSerie.resetData(new DataPoint[] {});
    ...
    {
        if(tagList.size() == 0) { return; }
        if(this.tagSelectedName == null) { return; }

        for(Map.Entry<String, Tag> entry : tagList.entrySet())
        {
            if(entry.getKey().equals(this.tagSelectedName) &&
                    (entry.getValue() instanceof TagTemperature))
            {
                TagTemperature tagT = (TagTemperature) entry.getValue();
                this.tagName.setText(entry.getKey());
                this.tagConnectName.setText(entry.getKey());
                this.tagValueTemp.setText(String.valueOf(tagT.getTemperature())+ " °C");
            }
        }
    }
