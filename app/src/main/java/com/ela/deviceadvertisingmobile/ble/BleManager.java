...
public class BleManager implements BleGattExecutor.BleExecutorListener {
    ...
    /** Bluetooth device from android.bluetooth */
    ...
    /** internal ble listener */
    private BleManagerListener mBleListener;

    /** singleton on Ble Manager
     * @param context context : target contect for connection
     */
    ...
    public boolean connect(Context context, String address) {
        ...
        if (mAdapter == null || address == null) {
            DebugText.getInstance().write(">>> ERROR : BluetoothAdapter not initialized or unspecified address");
        ...
        if (reuseExistingConnection) {
            ...
            if (mDeviceAddress != null && address.equalsIgnoreCase(mDeviceAddress) && mGatt != null) {
                DebugText.getInstance().write(">>> WARNING : Trying to use an existing BluetoothGatt for connection");
                ...
                if (mGatt.connect()) {
                    ...
                    DebugText.getInstance().write(">>> ERROR : Connection is refused");
                    return false;
                }
            }
        } else {
            ...
            if (forceCloseBeforeNewConnection) {
                DebugText.getInstance().write(">>> ERROR : Connection is now closed");
                close();
            }
        }
        ...
        if (mDevice == null) {
            DebugText.getInstance().write(">>> ERROR : Device not found. Unable to connect");
            Log.w(TAG, "Device not found.  Unable to connect.");
            return false;
        }

        DebugText.getInstance().write(">>> WARNING : Trying to create a new connection");
        ...
        mGatt = mDevice.connectGatt(context, false, mExecutor,BluetoothDevice.TRANSPORT_LE);
        return true;
    }
    ...
    /** Function to know if user can read the RSSI value from Tag
     * @return tru reading Rssi is available, false is not
     */
    ...
    public void writeService(BluetoothGattService service, String uuid, byte[] value) {
        if (service != null) {
            if (mAdapter == null || mGatt == null) {
                DebugText.getInstance().write(">>> ERROR : BluetoothAdapter not initialized");
                Log.w(TAG, "writeService: BluetoothAdapter not initialized");
                return;
            }
            DebugText.getInstance().write(">>> Write " + value.toString() + "on UUID " + uuid + "on Service " + service.toString());
            mExecutor.write(service, uuid, value);
            mExecutor.execute(mGatt);
        }
    }

    /** enable a notification Gatt service */
